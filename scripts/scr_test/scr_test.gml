...
function ken_scr_test_vsl(){
	var at_kscripts = Kengine.asset_types.vsl;
	var foo_kscript = new Kengine.Asset(at_kscripts, "kscr_foo", false, 1234);
	foo_kscript.src = "print_error(\"LOL!\");\n" + 
		"print(arguments);\n" +
	...
	return ken_execute_script(foo_kscript, ["55", "66"]);
}
    ...
    if not test.is_testing {
        ...
        var fixtures = [
            new Kengine.tests.Fixture("fixture-ball-object", function(){
                ...
                var ball_par_obj_index = at_objects.assets.get_ind("obj_test_ball_parent", Kengine.utils.cmps.cmp_val1_val2_index);
                if ball_par_obj_index == -1 {
                    ball_par_asset = new Kengine.Asset(at_objects, "obj_test_ball_parent", );
                }

                // Ball
                var ball_obj_index = at_objects.assets.get_ind("obj_test_ball", Kengine.utils.cmps.cmp_val1_val2_index);
                if ball_obj_index == -1 {
                    ball_asset = new Kengine.Asset(at_objects, "obj_test_ball", );
                    ball_asset.parent = ball_par_asset;
                    ball_asset.instance_var_struct = {
                ...
                // Instancing
                Kengine.tests.test_manager.ball_1 = ken_instance_create(Kengine.tests.test_manager.ball_asset, 50,50);
                Kengine.tests.test_manager.ball_2 = ken_instance_create(Kengine.tests.test_manager.ball_asset, 200,300);
				// test.ball_1.instance.sprite_index = spr_ball;
				// test.ball_2.instance.sprite_index = spr_ball;

                if ken_instance_exists(Kengine.tests.test_manager.ball_asset) {
...
function ken_scr_test_ball() {
	...
	var ball_obj_index = at_objects.assets.get_ind("obj_test_ball", Kengine.utils.cmps.cmp_val1_val2_name);
	...
	var ball_1 = ken_instance_create(new_ball, 50,50);

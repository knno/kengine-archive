...
function txr_parse(str) {
	...
	while (pos <= len) {
		...
		switch (char) {
			...
			case ord("."): ds_list_add(out, [txr_token.period, inf]); break; // NOTE: TXR - Modified for Kengine
			case ord("+"):
			...
			default:
				if (char >= ord("0") && char <= ord("9")) {
					...
					|| (char >= ord("A") && char <= ord("Z"))
					// || (char == ord(".")) // NOTE: TXR - Modified for Kengine
				) {
					while (pos <= len) {
						char = string_ord_at(str, pos);
						if (char == ord("_")
					...
					// name = Kengine.utils.parser.txr.dot2ident(name); // NOTE: TXR - Modified for Kengine
					switch (name) {

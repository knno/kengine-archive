...
#macro __TXR_FUNCTION_DEFAULT ken_txr_default_function
...
/// (identifier, locals) throws halt
#macro __TXR_FUNCTION_IDENT Kengine.utils.parser.txr.get_ident
/// (identifier, locals, value) throws halt
#macro __TXR_FUNCTION_SET_IDENT Kengine.utils.parser.txr.set_ident
/// (identifier, field, locals) throws halt
#macro __TXR_FUNCTION_GET_FIELD Kengine.utils.parser.txr.get_field
/// (identifier, field, value, locals) throws halt
#macro __TXR_FUNCTION_SET_FIELD Kengine.utils.parser.txr.set_field
#endregion
				...
				if (tkn[0] == __TXR_TOKEN.PAR_OPEN) { // `ident(`
					...
					if (_fn == undefined) {
						if (__TXR_VALUE_CALLS) {
							...
							_fn_script = __TXR_FUNCTION_DEFAULT;
							if (_fn_script != -1) {
								_fn_argc = -1;
...
function __TxrSerializer() constructor {
	...
	static _thread_read = function(b) {
		...
		//show_debug_message(_sfmt("locals@%", b.tell()));
		var m = ds_map_create();
		...
		//show_debug_message(_sfmt("actions@%", b.tell()));
		var n = buffer_read(b, buffer_u32);
		var w = array_create(n);
		for (var i = 0; i < n; i++) {
			w[i] = _action_read(b);
	...
	static _thread_write = function(th, b) {
		...
		//show_debug_message(_sfmt("stack@%", b.tell()));
		...
		while (pos < len) {
			...
			switch (q[0]) {
				...
				case __TXR_ACTION.SET_IDENT:
					...
					var f = __TXR_FUNCTION_SET_IDENT;
					if is_undefined(f) {
						self[$ q[2]] = value;
					} else {
						try {
				...
				case __TXR_ACTION.GET_FIELD:
					var v = ds_stack_pop(stack);
					var f = __TXR_FUNCTION_GET_FIELD;
					if is_undefined(v) {
				...
				case __TXR_ACTION.SET_FIELD:
					var v = ds_stack_pop(stack);
					var f = __TXR_FUNCTION_SET_FIELD;
					if is_undefined(v) {
						halt = _sfmt("Cannot access (%) of undefined", q[2]);
						break;
					} if is_undefined(f) {
...
// Create a reference to (new _TXR());

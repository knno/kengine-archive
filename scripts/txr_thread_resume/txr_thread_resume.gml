	...
	while (pos < len) {
		...
		switch (q[0]) {
			...
			case txr_action.binop:
				...
				break;
			case txr_action.ident: // NOTE: TXR - Modified for Kengine
				var ident = q[2];
				var s = Kengine.utils.parser.txr.ident2dot(ident);
				var vv = Kengine.utils.parser.txr.get_ident(ident, locals);
	            ds_stack_push(stack, vv);
				// ds_stack_push(stack, self[$ q[2]]);
				break;
			case txr_action.set_ident: // NOTE: TXR - Modified for Kengine
				ident = q[2];
				if not Kengine.utils.parser.txr.set_ident(ident, ds_stack_pop(stack), locals) {
					halt = txr_sfmt("% is not assignable.", q[2]);
				}
				// self[$ q[2]] = ds_stack_pop(stack);
				break;
			case txr_action.get_field: // NOTE: TXR - Modified for Kengine
				var v = ds_stack_pop(stack);
				if v == undefined {
					halt = txr_sfmt("Cannot access (%) of undefined", q[2]);
					break;
				}
				v = variable_instance_get(v, q[2]);
				ds_stack_push(stack, v);
				break;
			case txr_action.set_field: // NOTE: TXR - Modified for Kengine
				var v = ds_stack_pop(stack);
				if v == undefined {
					halt = txr_sfmt("Cannot access (%) of undefined", q[2]);
					break;

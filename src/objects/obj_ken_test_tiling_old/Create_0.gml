...
grid_width = 5; // Number of tiles in the grid
grid_height = 5;
tile_size = 16; // Tile size in texture
grid_tile_size = tile_size; // Size in real pixels
tileset_width = sprite_get_width(spr_my_mod_outside_tileset);
tileset_height = sprite_get_height(spr_my_mod_outside_tileset);
...
for (var _y = 0; _y < grid_height; _y++) {
    for (var _x = 0; _x < grid_width; _x++) {
		...
		//__kengine_log("x1 {0}, y1 {1}, x2 {2}, y2 {3}", (_x) * grid_tile_size, (_y) * grid_tile_size,(_x+1) * grid_tile_size, (_y+1) * grid_tile_size);
		//__kengine_log("u1 {0}, v1 {1}, u2 {2}, v2 {3}", tu, tv, tu2, tv2);
	}
}
...
draw_clear_alpha(c_black, 0);
draw_sprite(spr_my_mod_outside_tileset, 0, 0, 0);
surface_reset_target();
